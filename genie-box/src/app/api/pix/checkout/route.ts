...
export async function POST(req: Request) {
  ...
  if (!parsed.success) return NextResponse.json({ ok: false, error: "giftId obrigatório" }, { status: 400 });

  const gift = await db.gift.findUnique({ where: { id: parsed.data.giftId }, include: { owner: true } });
  if (!gift || gift.status !== "AVAILABLE") {
    return NextResponse.json({ ok: false, error: "Presente indisponível" }, { status: 400 });
  }

  const owner = gift.owner;
  if (!owner.pixKeyEnc) return NextResponse.json({ ok: false, error: "Este perfil não configurou Pix" }, { status: 400 });
